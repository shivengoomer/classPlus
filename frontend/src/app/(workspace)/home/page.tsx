...
export default function HomePage() {
  ...
  // fetch assignments and user profile concurrently on page load
  useEffect(() => {
    async function loadInitialData() {
      try {
        await Promise.all([
          listAssignments().then((data) => setAssignments(data)),
          fetchProfile()
        ]);
      } catch (err) {
  ...
  return (
    <AppShell>
      {loading ? (
        <div className="w-full max-w-6xl mx-auto flex flex-col gap-6 pb-16 px-[2px] relative z-10 select-none">
          {/* Overview Header Row Skeleton */}
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mt-2 select-none animate-pulse">
          ...
          {/* Grouped Metrics Block Skeleton */}
          <div className="backdrop-blur-md bg-white/70 border border-white/60 rounded-3xl p-6 grid grid-cols-2 md:grid-cols-4 gap-y-6 gap-x-4 shadow-[0_8px_30px_rgba(0,0,0,0.01)] animate-pulse">
            {[1, 2, 3, 4].map((i) => (
              <div key={i} className="flex flex-col justify-between min-h-[90px] px-2 first:pl-0 md:first:pl-2 first:border-0 border-l border-slate-200/60 first:border-l-0">
          ...
          {/* SaaS Bento Grid section Skeleton */}
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start mt-2 select-none animate-pulse">
            {/* Quick Starts (Col span 2) */}
            <div className="lg:col-span-2 flex flex-col gap-4">
              <div className="h-4 w-48 bg-slate-200 rounded pl-1" />
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {[1, 2, 3].map((i) => (
                  <div key={i} className="backdrop-blur-md bg-white/70 border border-white/60 rounded-3xl p-5 shadow-[0_8px_30px_rgba(0,0,0,0.01)] flex flex-col justify-between gap-4 min-h-[160px]">
                    <div className="flex flex-col gap-2">
                      <div className="flex items-center justify-between">
            ...
            <div className="flex flex-col gap-4">
              <div className="h-4 w-32 bg-slate-200 rounded pl-1" />
              <div className="backdrop-blur-md bg-white/70 border border-white/60 rounded-3xl p-5 shadow-[0_8px_30px_rgba(0,0,0,0.015)] flex flex-col gap-5 min-h-[300px]">
                {[1, 2, 3].map((i) => (
                  <div key={i} className="flex gap-3 items-start">
                    <div className="w-2.5 h-2.5 rounded-full bg-slate-200 mt-3 flex-shrink-0" />
                    <div className="flex-1 bg-white/40 border border-slate-100 p-3 rounded-2xl flex flex-col gap-2">
                      <div className="flex justify-between items-center">
                        <div className="h-4 w-24 bg-slate-200 rounded" />
                        <div className="h-3 w-10 bg-slate-200 rounded" />
                      </div>
                      <div className="h-3 w-32 bg-slate-150 rounded" />
                    </div>
                  </div>
                ))}
              </div>
        ...
        <div className="w-full max-w-6xl mx-auto flex flex-col gap-6 pb-16 px-[2px] relative z-10">
          ...
          >
            ...
            <div className="btn-wrapper scale-90 md:scale-100 origin-right">
              <button
                className="btn"
              ...
              >
                ...
                <svg className="btn-svg" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path d="M22 2L11 13M22 2L15 22L11 13M11 13L2 9L22 2Z" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
                </svg>
              </button>
          ...
          >
            ...
            >
              <div className="flex items-center justify-between">
                ...
                <div className="w-7 h-7 rounded-xl bg-[#10375C]/10 flex items-center justify-center text-[#10375C] group-hover:scale-110 transition-transform">
            ...
            >
              ...
              <div className="flex justify-between items-baseline mt-4">
                ...
                <span className="text-[9px] text-[#10375C] bg-[#10375C]/5 border border-[#10375C]/15 px-2 py-0.5 rounded-md font-bold uppercase">
            ...
            >
              <div className="flex items-center justify-between">
                ...
                <div className="w-7 h-7 rounded-xl bg-sky-500/10 flex items-center justify-center text-sky-650">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start mt-2">
            ...
            <div className="lg:col-span-2 flex flex-col gap-4">
              <h3 className="text-xs font-black text-slate-400 uppercase tracking-wider flex items-center gap-2 pl-1">
                <Cpu className="w-4 h-4 text-[#10375C]" />
              ...
              >
                    ...
                    >
                      <div className="flex flex-col gap-2">
                        <div className="flex items-center justify-between">
                          ...
                          <div className="w-6 h-6 rounded-full bg-slate-50 group-hover:bg-[#10375C]/5 flex items-center justify-center text-slate-400 group-hover:text-[#10375C] transition-colors">
                ...
                <motion.div
                  variants={itemVariants}
                  onClick={() => router.push('/create')}
                  className="hidden md:flex bg-white/50 border border-dashed border-[#10375C]/25 hover:border-[#10375C]/50 rounded-3xl p-5 shadow-sm transition-all cursor-pointer flex-col justify-center items-center gap-2 group hover:-translate-y-0.5 duration-300"
                >
                  <div className="p-3 rounded-full bg-[#10375C]/10 text-[#10375C] group-hover:scale-110 transition-transform">
                    <Sparkles className="w-5 h-5" />
                  </div>
                  <h4 className="text-xs font-black text-slate-850 mt-2">Custom AI Generation</h4>
                      ...
                      >
                        ...
                        <div className="flex-1 bg-white/40 group-hover:bg-white/80 border border-slate-100 hover:border-orange-500/10 p-3 rounded-2xl transition-all shadow-sm flex flex-col gap-0.5">
                          <div className="flex justify-between items-start gap-2">
                            <span className="text-xs font-bold text-slate-800 group-hover:text-[#10375C] transition-colors line-clamp-1">
                              {item.title}
                            </span>
